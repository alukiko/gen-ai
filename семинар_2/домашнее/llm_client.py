...
Ожидаемые переменные окружения:
  LLM_BASE_URL   — базовый URL (OpenRouter: https://openrouter.ai/api/v1)
  LLM_AUTH_TOKEN — bearer-токен (для OpenRouter: sk-or-v1-... из openrouter.ai/keys)
  LLM_MODEL      — slug модели (OpenRouter: openai/gpt-4o-mini, google/gemini-2.5-flash, …)

OpenRouter (опционально):
  OPENROUTER_API_KEY      — альтернатива LLM_AUTH_TOKEN
  OPENROUTER_HTTP_REFERER — заголовок HTTP-Referer (по умолчанию https://github.com)
  OPENROUTER_APP_NAME     — заголовок X-OpenRouter-Title
...
def _openrouter_default_headers() -> dict[str, str]:
    return {
        "HTTP-Referer": os.environ.get("OPENROUTER_HTTP_REFERER", "https://github.com"),
        "X-OpenRouter-Title": os.environ.get("OPENROUTER_APP_NAME", "gen-ai-dpo-homework"),
    }
...
def _make_openai_client() -> OpenAI:
    ...
    if base:
        ...
        if not key:
            raise RuntimeError(
                "Не задан ключ API. Для OpenRouter: LLM_AUTH_TOKEN или OPENROUTER_API_KEY в .env."
            )
        timeout = float(os.environ.get("LLM_TIMEOUT", "200"))
        # OpenRouter — публичный TLS; self-hosted стенды — verify=False
        verify_ssl = _is_openrouter(base)
        http = httpx.Client(verify=verify_ssl, timeout=timeout)
        kwargs: dict[str, Any] = {
            "api_key": key,
        ...
        if _is_openrouter(base):
            kwargs["default_headers"] = _openrouter_default_headers()
...
def get_model() -> str:
    ...
    if os.environ.get("LLM_BASE_URL") and _is_openrouter(os.environ["LLM_BASE_URL"]):
        ...
        **kw: Any,
        ...
        for _ in range(max_retries + 1):
            try:
                ...
                hint = (
                    f"Невалидный ответ: {e}. Верни ТОЛЬКО один корректный JSON по схеме. "
                    "Если ошибка про возраст и graduation_year: "
                    "age >= (текущий_год - graduation_year + 22), не занижай age."
                )
