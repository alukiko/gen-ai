...
5. Если исходный вопрос содержит ложную предпосылку (например, спрашивает
   «во сколько раз подорожал», а данные показывают снижение), НЕ бракуй
   корректный ответ только из-за слова пользователя. Прими ответ, если он
   честно говорит, что показатель не вырос, а снизился, и подтверждает это
   расчетом через calculate.
